...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      switch (method) {
        ...
        case 'chapa':
          console.log("Attempting Chapa payment...");
          try {
            const chapaResponse = await axios.post(backendUrl + '/api/order/chapa', orderData, {
              headers: { token }
            });
            console.log("Chapa response:", chapaResponse.data);
            
            if (chapaResponse.data.success && chapaResponse.data.checkout_url) {
  ...
  const PaymentOption = ({ method, setMethod, value, logo, text }) => {
    return (
      <div
        ...
        style={{
          ...
          padding: text ? '0.5rem 0.75rem' : '0.25rem 1rem',
          cursor: 'pointer',
          borderRadius: '10px',
          transition: 'all 0.2s ease',
          flex: isMobile ? '1 1 100%' : '0 1 auto',
          backgroundColor: method === value ? '#f0f0f0' : 'transparent',
        }}
      >
        <div style={{
          minWidth: '0.875rem',
          ...
          border: '1px solid #DDDDDD',
          borderRadius: '9999px',
          backgroundColor: method === value ? '#B6F500' : 'transparent',
          transition: 'background-color 0.2s ease',
        }}></div>

        {logo && <div style={{ margin: '0 0.35rem' }}>{logo}</div>}
        {text && <p style={{
          color: '#6b7280',
          fontSize: '0.875rem',
          fontWeight: 500,
        ...
        <div>
          ...
          <div style={{
            ...
            <PaymentOption
              ...
              value="stripe"
              logo={
                <svg width="3rem" height="1.25rem" viewBox="0 0 512 214">
            ...
            <PaymentOption
              ...
              value="chapa"
              logo={
                <img src={chapalogo} alt="Chapa" style={{ height: '1.75rem', width: '5rem' }} />
              }
            />
